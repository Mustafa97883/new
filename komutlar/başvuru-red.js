...
if (!message.member.roles.cache.has(yetkili)) return message.channel.send('Bu Komutu Kullanamazsın')    
  ...
  .setDescription(`<@${başvuruisim}> adlı kişinin yaptığı başvuru reddedildi.\nSebep : ${sebep}\nReddeden yetkili : ${message.author}`)
    ...
    if (!başvuruisim) return message.channel.send(`<a:alarm3:823900153831620658> başvuranın ID'sini yazmalısın.`).then(m => m.delete({timeout: 10000}))
  if (!sebep) return message.channel.send(`<a:alarm3:823900153831620658> başvuruyu neden onaylamadığını yazmalısın.`).then(m => m.delete({timeout: 10000}))
...
exports.conf = {
  enabled: true,
  guildOnly: false,
  aliases: ['başvuru-reddet', 'reddet','red'],
  permLevel: 3
};
